...
def check_retweet_status(obj):
    ...
    """
    return "retweeted_status" in obj and not obj["is_quote_status"]
...
def load_data(tweets_file):
    ...
    with open(file = os.path.join(input_data_dir, tweets_file), mode= "r") as read_file:
...
SOCIAL_SECURITY_2023 = ["ansiosidonnainen", "asuminen", "asumistuki", "ennaltaehkäisy", "esteettömyys", "huoltosuhde", "hyvinvointi", "hyvinvointiasuomeen", "ihmisarvo", "ihmisoikeudet", "lapset", "mielenterveys", "perhe", "perheystävällisyys", "synnytystalkoot", "synnytys", "syrjintä", "syrjäytyminen", "terveys", "vanhukset"] + SOCIAL_SECURITY_2019
EDUCATION_2023 = ["esiopetus", "koulu", "kulttuuri", "kulttuuriala", "kulttuuripolitiikka", "nuoret", "opehommat", "opettajanääni", "opetus", "opiskelijat", "oppivelvollisuus", "osaaminen", "s2-oppilas", "tiede"] + EDUCATION_2019

SDP_2023 = [] + SDP_2019
FINNS_2023 = ["ps2023", "sinivalkoinensiirtymä", "suomitakaisin"] + FINNS_2019
NATIONAL_2023 = ["oikeaaika", "kokkari", "kokkarit"] + NATIONAL_2019
CENTER_2023 = ["kepu", "keskustasekotimainen"] + CENTER_2019
GREEN_2023 = ["vihervasemmisto"] + GREEN_2019
LEFT_2023 = ["vihervasemmisto", "vassari", "vassarit"] + LEFT_2019
PARTIES_2023 = ["kristillisdemokraatit", "suomiluottaa", "piraatit", "piraattipuolue", "liikenyt"] + PARTIES_2019 + SDP_2023 + FINNS_2023 + NATIONAL_2023 + CENTER_2023 + GREEN_2023 + LEFT_2023

KEYWORD_MAP = {
                "CLIMATE_2019": CLIMATE_2023, 
                "IMMIGRATION_2019": IMMIGRATION_2023, 
                "ECONOMIC_POLICY_2019": ECONOMIC_POLICY_2023,
                "SOCIAL_SECURITY_2019": SOCIAL_SECURITY_2023,
                "EDUCATION_2019": EDUCATION_2023,
                "SDP_2019": SDP_2023,
                "FINNS_2019": FINNS_2023,
                "NATIONAL_2019": NATIONAL_2023,
                "CENTER_2019": CENTER_2023,
                "GREEN_2019": GREEN_2023,
                "LEFT_2019": LEFT_2023,
                "PARTIES_2019": PARTIES_2023
            }
...
input_data_dir = "/m/cs/scratch/networks/ecanet/elections/raw_tweets/finnish_election"
output_data_dir = "./keywords_non_universal_stream_processed"
network_context = KEYWORD_MAP[network_name]
network_context_str = network_name

def run_pipeline():

    logging.info(f"Starting data pipeline for {network_context_str}...")

    # Load data from input directory
    twitter_files = sorted(os.listdir(input_data_dir))
    
    KEYWORDS_SELECTED = preprocess_keywords(network_context)

    EDGE_DATA = []

    for tf in tqdm(twitter_files):
        ...
        for retweet in data:
            ...
            tweet_text = retweet["retweeted_status"]["text"]
  
            TEXT_TOKENS = preprocess_text(tweet_text)

            if check_relevancy(TEXT_TOKENS, KEYWORDS_SELECTED):
                retweeter_node = retweet["user"]["id"]
                retweeted_node = retweet["retweeted_status"]["user"]["id"]

                timestamp = retweet["timestamp"]
                dt_object = datetime.fromtimestamp(timestamp)
                formatted_timestamp = dt_object.strftime("%Y-%m-%dT%H:%M:%S.%fZ")

                EDGE_DATA.append((retweeter_node, retweeted_node, formatted_timestamp))
